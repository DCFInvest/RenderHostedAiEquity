...
def get_all_tickers():
    """Pull all NYSE & NASDAQ tickers using yfinance."""
    try:
        nyse = pd.read_csv("https://pkgstore.datahub.io/core/nyse/nyse_symbols_csv/data/ca5d9f9f789f8c7e90e9f9de7d6c49f5/nyse_symbols_csv.csv")
        nasdaq = pd.read_csv("https://pkgstore.datahub.io/core/nasdaq-symbols/nasdaq-symbols_csv/data/cf95d4e09e34f6c5b4b0a04f9305ed8b/nasdaq-symbols_csv.csv")
        tickers = pd.concat([nyse, nasdaq])
...
def calculate_dcf(ticker, discount_rate=0.10, growth_rate=0.05, years=5):
    """Calculate DCF per share using Yahoo Finance."""
    try:
...
def process_stocks_parallel(tickers, max_workers=20):
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_ticker):
            ...
            completed += 1
            if completed % 50 == 0:
                logger.info(f"Processed {completed}/{total} tickers, valid: {len(results)}")
    logger.info(f"Completed: {len(results)}/{total} valid stocks")
...
def initialize_stock_data():
    ...
    tickers = get_all_tickers()[:500]  # limit to 500 to avoid timeout
...
def status():
    if STOCK_CACHE is not None:
        return jsonify({'ready': True, 'stock_count': len(STOCK_CACHE)})
