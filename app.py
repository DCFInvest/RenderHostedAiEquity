...
def get_nyse_nasdaq_tickers():
    """Fetch all tickers from Yahoo Finance US exchanges"""
    try:
        sp500 = pd.read_html('https://en.wikipedia.org/wiki/List_of_S%26P_500_companies')[0]
        tickers = sp500['Symbol'].tolist()
        # You can expand here with other lists (nasdaq, nyse)
        return tickers
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(future_to_ticker):
            try:
                ...
                continue
    logger.info(f"Processed {len(results)}/{len(tickers)} stocks successfully")
...
def initialize_stock_data():
    ...
    stock_data = process_stocks_parallel(tickers[:100])  # limit to first 100 for speed
    if stock_data.empty:
    ...
    logger.info(f"Stock data initialized with {len(stock_data)} entries")

# ----------------- API Routes -----------------
@app.route('/api/status')
def status():
    return jsonify({
        'ready': STOCK_CACHE is not None,
        'stock_count': len(STOCK_CACHE) if STOCK_CACHE is not None else 0,
